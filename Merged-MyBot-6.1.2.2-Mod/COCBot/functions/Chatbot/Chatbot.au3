...
;~    $hGUI_ModChat = GUICreate("", $_GUI_MAIN_WIDTH - 28, $_GUI_MAIN_HEIGHT - 255 - 28, 5, 25, BitOR($WS_CHILD, $WS_TABSTOP), -1, $hGUI_MOD)
;~    GUISetBkColor($COLOR_WHITE, $hGUI_ModChat)
...
Func ChatbotSelectClanChat() ; select clan tab
   Click (290 , 37, 1) ; switch to clan
   If _Sleep(1000) Then Return
	  ;MouseClickDrag("primary", 100,600,150,200) ;scroll to bottom
	  Click ( 283, 38, 1) ; scroll to top
   If _Sleep(1000) Then Return
   Return True
EndFunc

Func ChatbotSelectGlobalChat() ; select global tab
   Click ( 52, 35, 1) ; switch to global
   If _Sleep(1000) Then Return
   Return True
EndFunc

Func ChatbotChatClose() ; close chat area
   Click ( 335, 414, 1) ; close chat
   waitMainScreen()
   Return True
EndFunc

Func ChatbotChatClanInput() ; select the textbox for clan chat
   Click ( 280, 720, 1); open text chat
